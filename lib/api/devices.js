...
Devices = (function () {
  ...
  Devices.prototype.get = function (deviceId, data) {
    var d = q.defer();
    if (deviceId && data) {
  ...
  // addIdentifier
  Devices.prototype.addIdentifier = function (deviceId, identifier, data) {
    var d = q.defer();
    if (deviceId && data) {
      if (identifier) {
        this.sendRequest('/devices/' + deviceId + '/identifiers/' + identifier, 'POST', d, data);
      }
      else {
  ...
  // remove
  Devices.prototype.removeIdentifier = function (deviceId) {
    var d = q.defer();
    if (deviceId) {
      this.sendRequest('/organizations/' + deviceId, 'DELETE', d);
    }
    else {
      d.reject('Bad Request. A device id must be supplied.');
    }
  ...
  Devices.prototype.addChannel = function (deviceId, channelId, data) {
    var d = q.defer();
    if (deviceId && channelId && data) {
      this.sendRequest('/devices/' + deviceId + '/channels/' + channelId, 'PUT', d, data);
    }
    else {
      d.reject('Bad Request. A device id, a channel id and channel data must be supplied.');
    }
    return d.promise;
  };
