...
module.exports = (function () {
   ...
   * @param  {Object|String}    path   The path of the items you wish to GET, this can be a string or a promise object
   ...
   */
  RequestBase.prototype.all = function (path, params) {
    var promiseFunc,
        allApiItems = [],
        apiPromise;
    if (rbnUtil.isObject(path)) {
    ...
    promiseFunc = function (promise) {
      return promise.then(function (resp) {
        var dataLength = resp.getDataLength();
        if (dataLength === 0) {
          return Promise.resolve(allApiItems);
        } else if (dataLength === 1) {
          allApiItems.push(resp.getData());
        } else {
          Array.prototype.push.apply(allApiItems, resp.getData());
        }
        if (resp.nextPage) {
