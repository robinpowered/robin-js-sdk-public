...
exports.constructRobinUrl = function (robinType, env) {
  var _robinUrl = '',
      ...
      _protocol = 'https://',
      expectedEnvs = ['test', 'staging'],
      expectedTypes = ['api', 'apps', 'grid'];

  if (!robinType) {
    throw new TypeError('`robinType` is a required parameter');
  } else {
    if (expectedTypes.indexOf(robinType) === -1) {
      throw new TypeError('Invalid value for `robinType`. Value can be one of: ' + expectedTypes.join(','));
    }
  }

  if (env) {
    if (expectedEnvs.indexOf(env) !== -1) {
      _env = '.' + env;
      if (env === 'test') {
        _protocol = 'http://';
      }
